<p align="center">
  <a href="https://github.com/v3xlabs/eth-icons">
    <picture>
      <source srcset="./typescript/public/eth-icons_logo.svg" media="(prefers-color-scheme: dark)">
      <img src="./typescript/public/eth-icons_logo_light.svg" alt="Open Lavatory" width="auto" height="60">
    </picture>
  </a>
</p>

<p align="center">
  A smol rust/typescript crate/library for fetching Ethereum asset icons from various sources.
</p>
<table>
    <thead>
        <tr>
        <th>Rust</th>
        <th>Typescript</th>
        </tr>
    </thead>
    <tbody>
        <tr>
        <td>
            <p>
                <a href="https://docs.rs/eth-icons"><img src="https://img.shields.io/badge/Docs.rs-blue?logo=rust&color=brown&style=flat" alt="Documentation"></a>
                <a href="https://crates.io/crates/eth-icons"><img src="https://img.shields.io/badge/Crate.io-yellow?logo=data:image/x-icon;base64%" alt="Crates.io"></a>
                <a href="https://github.com/v3xlabs/eth-icons"><img src="https://img.shields.io/badge/Repository-v3xlabs/eth--icons-blue?style=flat" alt="Repository"></a>
                <a href="#"><img src="https://img.shields.io/badge/License-LGPL--3.0-hotpink?style=flat" alt="License: LGPL-3.0"></a>
            </p>
        </td>
        <td>
            <p>
                <a href="https://www.npmjs.com/package/eth-icons"><img src="https://img.shields.io/badge/npm-eth--icons-red?logo=npm&style=flat" alt="npm"></a>
                <a href="https://github.com/v3xlabs/eth-icons"><img src="https://img.shields.io/badge/Repository-v3xlabs/eth--icons-blue?style=flat" alt="Repository"></a>
                <a href="#"><img src="https://img.shields.io/badge/License-LGPL--3.0-hotpink?style=flat" alt="License: LGPL-3.0"></a>
            </p>
        </td>
        </tr>
        <tr>
            <td>
                <p>Read <a href="./cargo">more</a></p>
            </td>
            <td>
                <p>Read <a href="./typescript">more</a></p>
            </td>
        </tr>
    </tbody>
</table>
